...
// 1) >
bool CoffeeMachine::operator >(CoffeeMachine& another)
{
    return(this->coffee > another.coffee);
}
// 2) <
bool CoffeeMachine::operator <(CoffeeMachine& another)
{
...
//5) >> (cin)
istream& operator >>(istream& is, CoffeeMachine& machine)
{
    cout << "\nСколько надо добавить кофе -> ";
    is >> machine.coffee;
    return is;
}

//6) << (cout)
ostream& operator <<(ostream& os, const CoffeeMachine& machine)
{
    cout << "\nКофе в контейнер -> " << machine.coffee;
    return os;
}
